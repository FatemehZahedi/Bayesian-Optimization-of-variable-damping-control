...
%blockvec = [32 32 32 30 30 30];
blockvec = [32 32 32 31 31 31];
iterationvec = [1 2 3 1 2 3];
for i = 1:length(blockvec)
        ...
        for trial = 1:ntrial
            ...
            Tableofmeasures(trial,12) = sqrt(tangentialovershoot^2 + normalovershoot^2);
...
Average(1,:,:) = mean(Avemeasures(1:3,:,:));
Average(2,:,:) = mean(Avemeasures(4:6,:,:));

[~,index] = min(Avemeasures(1:3,11,1));
minmeasures(1,:,:) = Avemeasures(index,:,:);
[~,index] = min(Avemeasures(4:end,11,1));
minmeasures(2,:,:) = Avemeasures(index+3,:,:);



percentageImproveave = (-(Average(2,:,1)- Average(1,:,1))./mean(Average(1:2,:,1)))*100;

percentageImproveave(5)= -percentageImproveave(5);
percentageImproveave(6) = -percentageImproveave(6);

percentageImprovemin = (-(minmeasures(2,:,1)- minmeasures(1,:,1))./mean(minmeasures(1:2,:,1)))*100;

percentageImprovemin(5)= -percentageImprovemin(5);
percentageImprovemin(6) = -percentageImprovemin(6);

mean(percentageImproveave([12 4 5 6 7 8]))
mean(percentageImprovemin([12 4 5 6 7 8]))

save(['/home/fzahedi/Fatemeh/Adaptive/AP_BO_newsetup/DATA/Subject',num2str(subnum),'/plots/Subject',num2str(subnum),'.mat'],'percentageImproveave','Average','minmeasures','percentageImprovemin','Avemeasures');
