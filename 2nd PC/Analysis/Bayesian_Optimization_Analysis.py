...
# ax2.plot_surface(gx, gy, mu.reshape(gx.shape),cmap='coolwarm', alpha=0.5, antialiased=False, label='$\mu_{1|2}$')
# ax2.plot_surface(gx, gy, quants[0].reshape(gx.shape),cmap='gray', alpha=0.2, antialiased=False, label='$2 \sigma_{1|2}$')
# ax2.plot_surface(gx, gy, quants[1].reshape(gx.shape),cmap='gray', alpha=0.2, antialiased=False)
# ax2.scatter(bo.x[:-1,0],bo.x[:-1,1],bo.y[:-1], c='k', label='y_2')
...
# ax3.plot_surface(gx, gy, ei.reshape(gx.shape),cmap='coolwarm', alpha=0.5, antialiased=False, label='$\mu_{1|2}$')
# ax3.scatter(new_proposal[0], new_proposal[1], np.arange(ei_min, ei_max, 0.000005).reshape(-1, 1), marker='.', c='k', lw=0.01, label="New proposal")
# plt.title("$EI(x)$")
# plt.xlabel('$b_{lb}$')
# plt.ylabel('$b_{ub}$')
...
# plt.scatter(bo.best_params['x'],bo.best_params['bub'], c='yellow', marker='*')
