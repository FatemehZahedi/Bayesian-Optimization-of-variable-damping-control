...
fileID = fopen('/home/fzahedi/Fatemeh/Adaptive/AP_BO_newsetup/config/Config.txt','r');
for i=1:3
    tline = fgetl(fileID);
end
tline = fgetl(fileID);
subnum = str2num(tline);
tline = fgetl(fileID);
blocknew = str2num(tline);
fclose(fileID);
%subnum = 3;
ntrial = 1;
...
for block = size(Objectivefunc,2)+1-(blocknew-1)*priornum : size(Objectivefunc,2)+1-(blocknew-1)*priornum % This should be changed based on iteration that we are in
    clear measures
...
Objectivefunc(:,end+1) = ftrialbased(end,:)';
