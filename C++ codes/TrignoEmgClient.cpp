...
TrignoEmgClient::TrignoEmgClient(std::string ipAddr){
	...
	/* Initialize backwards compatibility and upsampling commands */
	_cmd_b[2] = "BACKWARDS COMPATIBILITY?\r\n\r\n";
	_cmd_b[3] = "UPSAMPLE [ON]\r\n\r\n";
	_cmd_b[4] = "UPSAMPLING?\r\n\r\n";
	_cmd_b[5] = "RATE EMG?\r\n\r\n";
}
...
void TrignoEmgClient::CheckBackwards(int cmdNumber){
    /*
     * This function sends the backwards compatibility command through the comm port.The argument cmdNumber
     * specifies which command to send.  All commands are listed in the std::map
     * variable named _cmd_b
     */
	std::string cmd = _cmd_b[cmdNumber];
	if (cmd.compare("") != 0){	// command found
		printf("Command: %s\n", RemoveNewlines(cmd).c_str());
    	try{
          _sockComm.send(boost::asio::buffer(cmd.c_str(), strlen(cmd.c_str())));
